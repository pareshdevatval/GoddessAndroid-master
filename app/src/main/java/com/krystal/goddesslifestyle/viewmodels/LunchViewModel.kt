 ...
 */
class LunchViewModel(application: Application) : BaseViewModel(application) {
    /*RxJava Subscription object for api calling*/
    private var subscription: Disposable? = null

    private val lunchListResponse: MutableLiveData<RecipeListReponse> by lazy {
        MutableLiveData<RecipeListReponse>()
    }

    fun getLunchListResponseResponse(): LiveData<RecipeListReponse> {
        return lunchListResponse
    }
    ...
    fun callGetRecipeListAPI(pageNo: Int = 1,searchValue:String="",showProgress: Boolean = true) {
        if (AppUtils.hasInternet(getApplication())) {
            ...
            params[ApiParam.KEY_RECIPE_TYPE] = ApiContants.RECIPE_TYPE_LUNCH
    ...
    private fun handleResponse(response: RecipeListReponse) {
        lunchListResponse.value = response
    }
